...
import {
  getgooeyPackageMetadata,
  buildgooeyPackageDownloadLink,
} from "../services/gooey.api"
import { gooeyPackageMetadata } from "../types/gooey"
...
const DependencyLinkTooltip = styled.span`
  ...
  color: white;
  font-size: 0.8rem;
  background-color: var(--gooey-brown);
  border-radius: 5px;
  padding: 10px;
  top: -45px;
  left: 50%;
...
export default function Package() {
  ...
  const [packageHistory, setPackageHistory] = useState<[gooeyPackageMetadata]>()
  ...
  const loadPackageData = async (packageScope: string, packageName: string) => {
    const packageData = await getgooeyPackageMetadata(packageScope, packageName)
    ...
    const filteredPackageData = packageData.versions.some(
      (pack: gooeyPackageMetadata) => !pack.package.version.includes("-")
    )
  ...
  const packageMetadata = packageHistory?.find(
    (item: gooeyPackageMetadata) => item.package.version === packageVersion
  )
        ...
        <FlexColumns>
          ...
          <NarrowColumn>
            ...
            <MetaItem title="Version" width="half">
              ...
              >
                {packageHistory?.map((item: gooeyPackageMetadata) => {
                  return (
                    <option
            ...
            <MetaItem title="Download" width="half">
              <DownloadLink
                url={buildgooeyPackageDownloadLink(
                  packageScope,
                  packageName,
              ...
              >
                <img
                  src={iconDownload}
                  alt="Download"
                  style={{ fill: "var(--gooey-mauve)", height: "1rem" }}
                />
              </DownloadLink>
            </MetaItem>
            ...
            {/* TODO: Re-implement when gooey API supports custom source repos */}
