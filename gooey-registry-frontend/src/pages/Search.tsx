...
const SearchPackages = ({ packages }: { packages: gooeyPackageBrief[] }) => (
  <>
    {[...packages].map((pack: gooeyPackageBrief, index) => (
      <PackageTag
        size="large"
        width="wide"
        key={index}
...
export default function Search() {
  ...
  const [packages, setPackages] = useState<gooeyPackageBrief[]>()
  const [isLoaded, setIsLoaded] = useState(false)
  const [isError, setIsError] = useState(false)

  const loadPackagesListData = async (searchQuery: string) => {
    const packagesListData = await getgooeyPackages(searchQuery)
