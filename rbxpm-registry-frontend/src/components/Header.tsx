...
import logo from "../assets/rbxpm-logo.svg"
import { isCondensed, isMobile, isMobileSmall, notMobile } from "../breakpoints"
import { getrbxpmPackages } from "../services/rbxpm.api"
import { rbxpmPackageBrief } from "../types/rbxpm"
...
const StyledHeader = styled.header`
  background-color: var(--rbxpm-white);
  margin: 0 auto;
...
const StyledNav = styled.nav`
  ...
  & > a {
    ...
    &::after {
      ...
      position: absolute;
      height: 1.4rem;
      border-right: 2px solid var(--rbxpm-mauve);
      right: 0;
      top: 52%;
...
const reactSelectSearchTheme = (theme: any) => ({
  ...
  colors: {
    ...theme.colors,
    primary25: "var(--rbxpm-red-light)",
    primary: "var(--rbxpm-mauve)",
  },
})
...
const reactSelectSearchStyles = {
  ...
  control: (provided: any) => ({
    ...
    ":focus-within": {
      borderColor: "var(--rbxpm-red)",
      boxShadow: "0 0 0 1px var(--rbxpm-red)",
    },
  }),
}
...
const filterrbxpmPackages = async (inputValue: string) => {
  const packagesListData = await getrbxpmPackages(inputValue)
  const searchOptions = packagesListData.map(
    (packageBrief: rbxpmPackageBrief) => ({
...
export default function Header() {
  ...
  const loadOptions = async (inputValue: string) =>
    new Promise<rbxpmOption[]>((resolve) => {
      resolve(filterrbxpmPackages(inputValue))
    })

  const onChange = (option: rbxpmOption | null) => {
    if (option) {
