...
import {
  getrbxpmPackageMetadata,
  buildrbxpmPackageDownloadLink,
} from "../services/rbxpm.api"
import { rbxpmPackageMetadata } from "../types/rbxpm"
...
const DependencyLinkTooltip = styled.span`
  ...
  color: white;
  font-size: 0.8rem;
  background-color: var(--rbxpm-brown);
  border-radius: 5px;
  padding: 10px;
  top: -45px;
  left: 50%;
...
export default function Package() {
  ...
  const [packageHistory, setPackageHistory] = useState<[rbxpmPackageMetadata]>()
  ...
  const loadPackageData = async (packageScope: string, packageName: string) => {
    const packageData = await getrbxpmPackageMetadata(packageScope, packageName)
    ...
    const filteredPackageData = packageData.versions.some(
      (pack: rbxpmPackageMetadata) => !pack.package.version.includes("-")
    )
  ...
  const packageMetadata = packageHistory?.find(
    (item: rbxpmPackageMetadata) => item.package.version === packageVersion
  )
        ...
        <FlexColumns>
          ...
          <NarrowColumn>
            ...
            <MetaItem title="Version" width="half">
              ...
              >
                {packageHistory?.map((item: rbxpmPackageMetadata) => {
                  return (
                    <option
            ...
            <MetaItem title="Download" width="half">
              <DownloadLink
                url={buildrbxpmPackageDownloadLink(
                  packageScope,
                  packageName,
              ...
              >
                <img
                  src={iconDownload}
                  alt="Download"
                  style={{ fill: "var(--rbxpm-mauve)", height: "1rem" }}
                />
              </DownloadLink>
            </MetaItem>
            ...
            {/* TODO: Re-implement when rbxpm API supports custom source repos */}
