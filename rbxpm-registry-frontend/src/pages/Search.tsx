...
const SearchPackages = ({ packages }: { packages: rbxpmPackageBrief[] }) => (
  <>
    {[...packages].map((pack: rbxpmPackageBrief, index) => (
      <PackageTag
        size="large"
        width="wide"
        key={index}
...
export default function Search() {
  ...
  const [packages, setPackages] = useState<rbxpmPackageBrief[]>()
  const [isLoaded, setIsLoaded] = useState(false)
  const [isError, setIsError] = useState(false)

  const loadPackagesListData = async (searchQuery: string) => {
    const packagesListData = await getrbxpmPackages(searchQuery)
