...
impl WallyPackageName {
        ...
        N: Into<String>,
    {
        ...
        Ok(WallyPackageName { scope, name })
    }
...
impl fmt::Display for WallyPackageName {
...
impl FromStr for WallyPackageName {
    ...
    fn from_str(value: &str) -> anyhow::Result<Self> {
        ...
        WallyPackageName::new(scope.to_owned(), name.to_owned())
    }
}

impl Serialize for WallyPackageName {
...
impl<'de> Deserialize<'de> for WallyPackageName {
...
mod test {
    ...
    #[test]
    fn new() {
        let package = WallyPackageName::new("flub-flab", "sisyphus-simulator-2").unwrap();
    ...
    fn new_invalid() {
        ...
        assert!(WallyPackageName::new("Upper-Skewer-Case", "Foo").is_err());

        // Underscores are not allowed to prevent confusion with dashes.
        assert!(WallyPackageName::new("snake_case", "foo").is_err());

        // Slashes are not allowed to avoid ambiguity.
        assert!(WallyPackageName::new("hello/world", "from/me").is_err());

        // Scopes and names must have one or more characters.
        assert!(WallyPackageName::new("", "").is_err());
    }

    #[test]
    fn parse() {
        let adopt_me: WallyPackageName = "flub-flab/sisyphus-simulator".parse().unwrap();
        assert_eq!(adopt_me.scope(), "flub-flab");
        assert_eq!(adopt_me.name(), "sisyphus-simulator");

        let numbers: WallyPackageName = "123/456".parse().unwrap();
    ...
    fn parse_invalid() {
        ...
        let extra: Result<WallyPackageName, _> = "hello/world/foo".parse();
        assert!(extra.is_err());
    }

    #[test]
    fn display() {
        let package_name = WallyPackageName::new("evaera", "promise").unwrap();
        assert_eq!(package_name.to_string(), "evaera/promise");
    }

    #[test]
    fn serialization() {
        let package_name = WallyPackageName::new("lpghatguy", "asink").unwrap();
        let serialized = serde_json::to_string(&package_name).unwrap();
        assert_eq!(serialized, "\"lpghatguy/asink\"");

        let deserialized: WallyPackageName = serde_json::from_str(&serialized).unwrap();
