...
use crate::package_compat::wally_package_req::WallyPackageReq;

pub const WALLY_MANIFEST_FILE_NAME: &str = "wally.toml";
...
pub struct WallyManifest {
    ...
    #[serde(default)]
    pub place: WallyPlaceInfo,

    #[serde(default)]
    pub dependencies: BTreeMap<String, WallyPackageReq>,

    #[serde(default)]
    pub server_dependencies: BTreeMap<String, WallyPackageReq>,

    #[serde(default)]
    pub dev_dependencies: BTreeMap<String, WallyPackageReq>,
}

impl WallyManifest {
    /// Load a manifest from a project directory containing a `wally.toml` file.
    pub fn load(dir: &Path) -> anyhow::Result<Self> {
        let file_path = dir.join(WALLY_MANIFEST_FILE_NAME);
    ...
    pub fn package_id(&self) -> WallyPackageId {
        WallyPackageId::new(self.package.name.clone(), self.package.version.clone())
    }
}
...
impl Default for WallyPlaceInfo {
    fn default() -> Self {
        Self {
