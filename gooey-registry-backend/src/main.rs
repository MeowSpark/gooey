...
fn root() -> content::RawJson<serde_json::Value> {
    content::RawJson(json!({
        "message": "gooey Registry is up and running!",
    }))
}
...
async fn package_contents(
    ...
    name: String,
    version: String,
    _cli_version: Result<gooeyVersion, Error>,
...
#[rocket::async_trait]
impl<'r> FromRequest<'r> for gooeyVersion {
    type Error = Error;
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        ...
        let minimum_version = match &config.minimum_gooey_version {
            Some(version) => version,
            None => return Outcome::Success(gooeyVersion),
        };

        let version = match request.headers().get_one("gooey-Version") {
            Some(version) => version,
            None => {
                return format_err!(
                    "gooey version header required. Try upgrading your gooey installation."
                )
        ...
        let version = match Version::parse(version) {
            Ok(version) => version,
            Err(err) => {
                return format_err!("Failed to parse gooey version header: {}", err)
                    .status(Status::BadRequest)
                    .into();
            }
        };
        ...
        if &version < minimum_version {
            format_err!(
                "This registry requires gooey {} (you are using {})",
                minimum_version,
                version
            )
            .status(Status::UpgradeRequired)
...
fn rocket() -> _ {
    let figment = Figment::from(rocket::Config::default())
        ...
        .merge(Env::prefixed("gooey_").global());
